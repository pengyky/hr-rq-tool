...
5. 检查 favicon 是否显示
6. 检查分享图资源 `og-image.png`（及备用矢量图 `og-image.svg`）是否能直接访问

## 六、准备正式分享前建议

1. 如果要长期公开使用，建议绑定自定义域名
2. 建议在社交平台或聊天工具里测试链接预览效果
3. 链接预览分享图已提供 PNG 版本 `og-image.png`（1200x630），并在 `index.html` 中配置为 `summary_large_image` 大图卡片；如需更新视觉，编辑 `og-image.svg` 后重新导出同名 PNG 即可
